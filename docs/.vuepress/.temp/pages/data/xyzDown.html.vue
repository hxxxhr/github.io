...
<p>目前我们所见的所有地图底图服务都是瓦片地图的方式发布的。瓦片地图金字塔模型是一种多分辨率层次模型，从瓦片金字塔的底层到顶层，分辨率越来越低，但表示的地理范围不变。</p>
<p>当我们建立好了影像金字塔后，前端再请求地图时，则将只是在切好的瓦片缓存中，找到对应级别里对应的瓦片即可。然后在前端将这些请求到的瓦片拼接出来，便可以得到用户需要的级别下的可视范围内的瓦片了。
<img src="@source/public/image/map-tileLayer-lod.jpg" alt="配置图"></p>
<h3 id="地图-url-的-xyz-参数" tabindex="-1"><a class="header-anchor" href="#地图-url-的-xyz-参数" aria-hidden="true">#</a> 地图 URL 的 XYZ 参数</h3>
<p>知道了地图是有很多瓦片组成，而每一张瓦片有对应的瓦片等级（z）和瓦片坐标编号（x, y），目前互联网地图也是通过 xyz 值是可以找到对应的瓦片的。</p>
<p>常见互联网地图的 xyz 地址：</p>
<p>高德地图: <code v-pre>http://webrd01.is.autonavi.com/appmaptile?lang=zh_cn&amp;size=1&amp;scale=1&amp;style=8&amp;x={x}&amp;y={y}&amp;z={z}</code></p>
<p>高德影像: <code v-pre>http://webst01.is.autonavi.com/appmaptile?style=6&amp;x={x}&amp;y={y}&amp;z={z}</code></p>
<p>Google 地图:<code v-pre> http://mt1.google.cn/vt/lyrs=m@207000000&amp;hl=zh-CN&amp;gl=CN&amp;src=app&amp;x={x}&amp;y={y}&amp;z={z}&amp;s=Galile</code></p>
<p>Google 影像: <code v-pre>http://mt1.google.cn/vt/lyrs=s&amp;hl=zh-CN&amp;gl=CN&amp;x={x}&amp;y={y}&amp;z={z}&amp;s=Gali</code></p>
<p>知道了 xyz 的地址后，我们平台的代码中就可以使用<a href="http://mars2d.cn/api/TileLayer.html" target="_blank" rel="noopener noreferrer">TileLayer<ExternalLinkIcon/></a>类来加载地图，例如加载瓦片可以这样写：</p>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h2 id="_2-下载互联网地图数据" tabindex="-1"><a class="header-anchor" href="#_2-下载互联网地图数据" aria-hidden="true">#</a> 2. 下载互联网地图数据</h2>
<p>知道了 xyz 的概念，我们就很容易理解去如何下载地图数据了，就是根据需要的地区范围确定 xyz 的范围值，轮询下载就可以了，自己写工具要考虑很多细节问题，不用重复造轮子， 目前市场上有很多相关工具。<br />
免费工具：<a href="https://mobac.sourceforge.io/" target="_blank" rel="noopener noreferrer"> MobileAtlasCreator<ExternalLinkIcon/></a> 
<a href="https://github.com/ShareQiu1994/node-map-download" target="_blank" rel="noopener noreferrer">Github 开源的代码工具<ExternalLinkIcon/></a><br />
...
<img src="@source/public/image/data-xyzDown-sjz4.jpg" alt="配置图"><br /> 5.下载完成后选择 “导出”<br />
<img src="@source/public/image/data-xyzDown-sjz5.jpg" alt="配置图"><br /> 6.弹出的窗口中按下图所示设置参数【重要】。<br />
<img src="@source/public/image/data-xyzDown-sjz6.jpg" alt="配置图"><br /> 7.导出完成后软件会自动打开导出的目录文件夹，会出现相应的数字目录即该下载的瓦片数据<br />
<img src="@source/public/image/data-xyzDown-sjz7.jpg" alt="配置图"><br /> 8.在数据量比较大的情况，直接拷贝切片会很慢很慢。一般操作是分层压缩成压缩包（并按 4G 分卷），拷贝至服务器后再进行解压。<br />
...
<p>可以在 Mars2D 框架中打开<a href="http://mars2d.cn/editor.html?id=layer-tile/type/xyz" target="_blank" rel="noopener noreferrer">XYZ 瓦片示例页面<ExternalLinkIcon/></a>，在这个页面的根据已有示例修改 url 地址<code v-pre> http://localhost:8888/mapdata/exp/{z}/{x}/{y}.jpg</code>，来调试图层参数。</p>
<p>可以参考教程<a href="http://mars2d.cn/doc.html#data/guide/map/tileLayer" target="_blank" rel="noopener noreferrer">加载 XYZ 数据步骤<ExternalLinkIcon/></a></p>
</div></template>
