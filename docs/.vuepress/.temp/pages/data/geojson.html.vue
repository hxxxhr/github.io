...
<h2 id="_1-shp格式介绍" tabindex="-1"><a class="header-anchor" href="#_1-shp格式介绍" aria-hidden="true">#</a> 1. shp格式介绍</h2>
<p>Shapefile文件是ESRI公司ArcGIS平台的常用格式文件，是工业标准的矢量数据文件。 Shapefile将空间特征表中的非拓扑几何对象和属性信息存储在数据集中，特征表中的几何对象存为以坐标点集表示的图形文件—SHP文件，Shapefile文件并不含拓扑（Topological）数据结构。<br />
<img src="@source/public/image/data-shp.jpg" alt="配置图"> <br />
shp一些特性：</p>
<ul>
<li>1个Shape文件至少包括三个文件：主文件(.shp) , 索引文件(.shx), dBASE表(*.dbf)</li>
<li>一个shp文件只能存储点、线、面中的一种类型，不存在混合存在的状态；</li>
<li>shp可以设置很多字段属性</li>
</ul>
<h2 id="_2-shp数据来源" tabindex="-1"><a class="header-anchor" href="#_2-shp数据来源" aria-hidden="true">#</a> 2. shp数据来源</h2>
...
<h2 id="_4-geojson格式介绍" tabindex="-1"><a class="header-anchor" href="#_4-geojson格式介绍" aria-hidden="true">#</a> 4. geojson格式介绍</h2>
<p>GeoJSON是适合于Web下对各种地理数据结构进行编码的格式，基于Javascript对象表示法的地理空间信息数据交换格式。与普通json文件格式区别在于对其属性及组成由一定规范。</p>
<p>GeoJSON将所有的地理要素分为Point、MultiPoint、LineString、MultiLineString、Polygon、MultiPolygon、GeometryCollection。首先是将这些要素封装到单个的geometry里，然后作为一个个的Feature（要素）；要素放到一个要素集合里构成。
<img src="@source/public/image/data-geojson.jpg" alt="配置图"></p>
<h2 id="_5-处理工具-gdal数据转换工具" tabindex="-1"><a class="header-anchor" href="#_5-处理工具-gdal数据转换工具" aria-hidden="true">#</a> 5.处理工具：GDAL数据转换工具</h2>
<p>shp转geojson有很多工具和方式，我们推荐的是GDAL转换方法</p>
<h3 id="_5-1-安装gdal程序" tabindex="-1"><a class="header-anchor" href="#_5-1-安装gdal程序" aria-hidden="true">#</a> 5.1. 安装GDAL程序</h3>
<p>下载<a href="data.mars2d,cn/tool/gdal-300-1911-x64-core.msi">gdal安装程序</a>，双击安装，一直默认即可，默认安装路径一般为<code v-pre>C:\Program Files\GDAL</code>，安装完成后可以打开浏览检查下。</p>
<h3 id="_5-2-配置环境变量" tabindex="-1"><a class="header-anchor" href="#_5-2-配置环境变量" aria-hidden="true">#</a> 5.2. 配置环境变量</h3>
<ul>
<li>（1）增加系统变量GDAL_DATA ，其值为 <code v-pre>C:\Program Files\GDAL\gdal-data</code></li>
<li>（2）增加系统变量PROJ_LIB ，其值为 <code v-pre>C:\Program Files\GDAL\projlib</code></li>
<li>（3）修改Path变量，在Path值中增加 <code v-pre>C:\Program Files\GDAL;</code></li>
</ul>
<h3 id="_5-3-转换shp为geojson命令" tabindex="-1"><a class="header-anchor" href="#_5-3-转换shp为geojson命令" aria-hidden="true">#</a> 5.3. 转换shp为geojson命令</h3>
<p>org2ogr是OGR模块中提供的一个重要工具，用于对数据源进行格式转换，打开cmd命令行，参考下面输入命令：<br />
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><p>命令（支持中文 + 输出wgs84经纬度）：</p>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><p><a href="https://gdal.org/programs/ogr2ogr.html" target="_blank" rel="noopener noreferrer">GDAL命令帮助文档<ExternalLinkIcon/></a></p>
...
<p>(7)批量转换</p>
<p>将postgis中所有的表都导出到mydatadump文件夹下，导出格式是ESRI Shapefile</p>
...
部分导出，将指定的表导出到mydatadump中，格式为ESRI Shapefile</p>
...
<p>导入指定的featureclass，重投影，重命名geometry列</p>
...
<p>下面是如何将shp和tab文件导入到postgis数据库中的命令方式</p>
...
<img src="@source/public/image/data-geojson-1.jpg" alt="配置图">
2.点击“确定”进入到下载界面 <br />
<img src="@source/public/image/data-geojson-2.jpg" alt="配置图"> <br />
3.下载完成后数据结构如下所示：
<img src="@source/public/image/data-geojson-3.jpg" alt="配置图"></p>
<h3 id="_6-2-shp格式转为geojson" tabindex="-1"><a class="header-anchor" href="#_6-2-shp格式转为geojson" aria-hidden="true">#</a> 6.2 shp格式转为geojson</h3>
<p>下面我们以 GDAL 数据转换工具为例，转换前确保已经按上面步骤安装好GDAL。</p>
<p>1.cdm（win + R） 打开命令窗口，进入 GDAL 的目录； 或者直接打开 GDAL 下的命令窗口
<img src="@source/public/image/data-geojson-4.jpg" alt="配置图">
2.输入命令，进行转换 <br /></p>
...
或者放在独立站点服务，可以参考<a href="">发布地图数据服务</a>。</p>
